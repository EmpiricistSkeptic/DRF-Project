...
class TestHabitViewSet:
    ...
    def test_list_habits_authenticated_returns_only_own(
        self, auth_client, user, active_habit, other_user_habit
    ):
        url = reverse("habit-list")
    ...
    def test_retrieve_other_user_habit_raises_404(
        self, auth_client, user, other_user_habit
    ):
    ...
    def test_retrieve_non_existent_habit_raises_404(self, auth_client, user):
        non_pk = (
            UserHabit.objects.last().pk if UserHabit.objects.exists() else 0
        ) + 999
    ...
    def test_create_habit_authenticated_invalid_data(self, auth_client, user):
        payload = {"description": "TestDesc", "frequency": "Daily"}
        url = reverse("habit-list")
        response = auth_client.post(url, data=payload)
    ...
    def test_update_own_habit_authenticated_success(
        self, auth_client, user, active_habit
    ):
        url = reverse("habit-detail", kwargs={"pk": active_habit.pk})
    ...
    def test_update_own_habit_authenticated_invalid_data(
        self, auth_client, user, active_habit
    ):
        url = reverse("habit-detail", kwargs={"pk": active_habit.pk})
    ...
    def test_update_other_user_habit_raises_404(
        self, auth_client, user, other_user_habit
    ):
        url = reverse("habit-detail", kwargs={"pk": other_user_habit.pk})
    ...
    def test_update_non_existent_habit_raises_404(self, auth_client, user):
        ...
        response = auth_client.patch(
            url, data={"title": "Attempt to update non-existent"}
        )
        assert response.status_code == status.HTTP_404_NOT_FOUND
    ...
    def test_delete_own_habit_authenticated_success(
        self, auth_client, user, active_habit
    ):
        url = reverse("habit-detail", kwargs={"pk": active_habit.pk})
    ...
    def test_list_habits_shows_only_active_habits(
        self, auth_client, user, active_habit, inactive_habit
    ):
        url = reverse("habit-list")
        response = auth_client.get(url)
        ids = (q["id"] for q in response.data["results"])
    ...
    def test_list_habits_pagination_works(self, auth_client, user, active_habit):
        ...
        ), "Ключ 'results' должен содержать список"
    ...
    def test_track_own_habit_already_tracked_today(
        self, auth_client, user, habit_untracked
    ):
        url = reverse("habit-track", kwargs={"pk": habit_untracked.pk})
    ...
    def test_track_habit_resets_streak_if_day_missed(
        self, auth_client, user, active_habit
    ):
