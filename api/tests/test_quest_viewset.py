...
class TestQuestViewSet:
    ...
    def test_list_quests_authenticated_returns_only_own(
        self,
        client,
        user,
        test_profile,
    ...
    def test_retrieve_other_user_quest_raises_404(
        self, client, user, test_profile, other_user_quest
    ):
        client.force_authenticate(user=user)
    ...
    def test_complete_active_quest_success(
        self, client, user, test_profile, active_quest
    ):
        ...
        assert (
            test_profile.points == initial_profile_points + active_quest.reward_points
        )
        assert test_profile.level == initial_profile_level

    def test_complete_already_completed_quest_fails(
        self, client, user, test_profile, completed_quest
    ):
        client.force_authenticate(user=user)
    ...
    def test_complete_other_user_quest_fails(
        self, client, user, test_profile, other_user_quest
    ):
        client.force_authenticate(user=user)
    ...
    def test_complete_quest_no_profile_fails(
        self, client, user, test_profile, active_quest
    ):
        client.force_authenticate(user=user)
