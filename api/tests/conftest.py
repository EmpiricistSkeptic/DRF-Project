...
def habit_untracked(user):
    return HabitFactory(
        user=user, title="Untracked", is_active=True, streak=0, last_tracked=None
    )
...
def other_user_habit(another_user):
    return HabitFactory(
        user=another_user, title="Yoga nidra 10 minutes", is_active=True, streak=3
    )
...
def active_quest(user):
    return QuestFactory(
        user=user, title="Study Spanish", status="ACTIVE", reward_points=500
    )
...
def completed_quest(user):
    return QuestFactory(
        user=user, title="Study English", reward_points=200, status="COMPLETED"
    )
...
def other_user_quest(another_user):
    return QuestFactory(
        user=another_user, title="Go for a run", status="ACTIVE", reward_points=50
    )
