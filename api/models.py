...
class Task(models.Model):
    ...
    def __str__(self):
        title_display = (
            self.title[:50] if self.title and self.title.strip() else "Без названия"
        )
        description_display = (
...
class Quest(models.Model):
    ...
    status = models.CharField(
        max_length=20, choices=QUEST_STATUS, default="ACTIVE", verbose_name="Статус"
    )
    reward_points = models.IntegerField(default=0, verbose_name="Награда XP")
    reward_other = models.CharField(
        max_length=255, blank=True, null=True, verbose_name="Другая награда (текст)"
    )
    penalty_info = models.CharField(
        max_length=255, blank=True, null=True, verbose_name="Штраф (текст)"
    )
    generated_at = models.DateTimeField(
        default=timezone.now, verbose_name="Время генерации"
    )
    expires_at = models.DateTimeField(
        blank=True, null=True, verbose_name="Срок выполнения (для срочных)"
    )
...
class UserAchievement(models.Model):
    ...
    current_tier = models.CharField(
        max_length=10, choices=Achievement.TIER_CHOICES, default="BRONZE"
    )
    completed = models.BooleanField(default=False)
...
class ChatHistory(models.Model):
    ...
    prompt_sent = models.TextField(
        verbose_name="Промпт, отправленный ИИ", blank=True, null=True
    )
    scenario = models.CharField(
        max_length=50, verbose_name="Определенный сценарий", blank=True, null=True
    )
    timestamp = models.DateTimeField(default=timezone.now, verbose_name="Время обмена")
    error_occurred = models.BooleanField(default=False, verbose_name="Произошла ошибка")
    error_message = models.TextField(
        verbose_name="Сообщение об ошибке", blank=True, null=True
    )
...
class UserHabit(models.Model):
    ...
    streak = models.PositiveIntegerField(default=0, verbose_name="Текущий стрик (дней)")
    ...
    last_tracked = models.DateField(
        null=True, blank=True, verbose_name="Дата последней отметки"
    )
    icon = models.CharField(
        max_length=50, blank=True, null=True, default="list-ul", verbose_name="Иконка"
    )
    ...
    def track_habit(self):
        ...
        if self.last_tracked == today:
            logger.warning(
                f"Habit '{self.title}' (ID: {self.id}) already tracked today ({today})."
            )
            return False
        ...
        if self.last_tracked:
            if today == self.last_tracked + timedelta(days=1):
                self.streak += 1
                logger.info(
                    f"Habit '{self.title}' (ID: {self.id}) streak continued: {self.streak} days."
                )
            elif today > self.last_tracked + timedelta(days=1):
                self.streak = 1
                logger.info(
                    f"Habit '{self.title}' (ID: {self.id}) streak reset to 1 after a gap."
                )
        else:
            self.streak = 1
            logger.info(
                f"Habit '{self.title}' (ID: {self.id}) tracked for the first time. Streak: 1."
            )
