...
def get_calories(request):
    ...
    if not product_name or not weight:
        ...
        return Response(
            {"error": "Product name and weight are required."},
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    body = {"query": query, "timezone": "Europe/Ukraine"}
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return Response(
                {
                    "error": f"Failed to fetch data from Nutritionix. Status code: {response.status_code}"
                },
