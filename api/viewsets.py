...
from .serializers import (
    ...
    ConsumedCaloriesSerializer,
    UserNutritionGoalSerializer,
...
class TaskViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        logger.info(
            f"Найден queryset из {queryset.count()} задач для пользователя {user.username}"
        )
        return queryset
    ...
    def list(self, request, *args, **kwargs):
        ...
        logger.info(
            f"Фильтрация списка задач: показ только невыполненных для {request.user.username}"
        )
        ...
        if page is not None:
            ...
            logger.debug(
                f"Возвращается пагинированный список невыполненных задач для {request.user.username}"
            )
            return self.get_paginated_response(serializer.data)

        serializer = self.get_serializer(queryset, many=True)
        logger.debug(
            f"Возвращается полный список невыполненных задач для {request.user.username}"
        )
    ...
    def complete(self, request, pk=None):
        ...
        if task.completed:
            logger.warning(
                f"Попытка повторно завершить уже выполненную задачу id={task.id} пользователем {request.user.username}"
            )
        ...
        try:
            with transaction.atomic():
                ...
                task.save()
                logger.info(
                    f"Задача id={task.id} помечена как выполненная пользователем {request.user.username}"
                )
                ...
                try:
                    ...
                    logger.error(
                        f"Не найден профиль для пользователя {request.user.username} при завершении задачи id={task.id}"
                    )
                    return Response(
            ...
            logger.exception(
                f"Ошибка при завершении задачи id={task.id} пользователем {request.user.username}: {e}"
            )
            return Response(
    ...
    def completed(self, request, *args, **kwargs):
        ...
        if page is not None:
            serializer = self.get_serializer(page, many=True)
            logger.debug(
                f"Возвращается пагинированный список ВЫПОЛНЕННЫХ задач для {request.user.username}"
            )
            return self.get_paginated_response(serializer.data)

        serializer = self.get_serializer(queryset, many=True)
        logger.debug(
            f"Возвращается полный список ВЫПОЛНЕННЫХ задач для {request.user.username}"
        )
        return Response(serializer.data)
...
class QuestViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def complete(self, request, pk=None):
        ...
        if quest_to_complete.status != "ACTIVE":
            logger.warning(
                f"Попытка завершить неактивный квест id={pk} пользователем {user.username}. Статус: {quest_to_complete.status}"
            )
            return Response(
                {
        ...
        try:
            with transaction.atomic():
                try:
                    profile = Profile.objects.select_for_update().get(
                        user=quest_to_complete.user
                    )
                except Profile.DoesNotExist:
                    logger.error(
                        f"КРИТИЧЕСКАЯ ОШИБКА: Профиль для пользователя {quest_to_complete.user.username} (id={quest_to_complete.user.id}) не найден при попытке завершить квест {quest_to_complete.id}."
                    )
                    return Response(
                ...
                if reward_points > 0:
                    ...
                    logger.info(
                        f"Игроку {user.username} начислено {reward_points} Points за квест '{quest_to_complete.title}'. Очки до: {original_points}, после: {profile.points}"
                    )

                    xp_threshold = int(1000 * (1.5 ** (profile.level - 1)))
                    if xp_threshold <= 0:
                        xp_threshold = 1000  # Защита
                    ...
                    while profile.points >= xp_threshold:
                        ...
                        profile.level += 1
                        logger.info(
                            f"Игрок {user.username} ДОСТИГ УРОВНЯ {profile.level}!"
                        )
                        xp_threshold = int(1000 * (1.5 ** (profile.level - 1)))
                        if xp_threshold <= 0:
                    ...
                    if leveled_up:
                        logger.info(
                            f"Обновлен профиль {user.username} после левел-апа: Уровень {original_level}->{profile.level}, Очки {original_points}->{profile.points}."
                        )
                ...
                logger.info(
                    f"Квест id={quest_to_complete.id} '{quest_to_complete.title[:30]}...' успешно завершен пользователем {user.username}."
                )
            ...
            logger.exception(
                f"Ошибка при завершении квеста {pk} для пользователя {user.username} внутри транзакции: {e}"
            )
            return Response(
...
class HabitViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        logger.info(
            f"Найден queryset из {queryset.count()} привычек для пользователя {user.username}"
        )
    ...
    def list(self, request, *args, **kwargs):
        ...
        logger.info(
            f"Фильтрация списка привычек: показ только активных для {request.user.username}"
        )
        ...
        if page is not None:
            serializer = self.get_serializer(page, many=True)
            logger.debug(
                f"Возвращается пагинированный список активных привычек для {request.user.username}"
            )
            return self.get_paginated_response(serializer.data)

        serializer = self.get_serializer(queryset, many=True)
        logger.debug(
            f"Возвращается полный список активных привычек для {request.user.username}"
        )
        return Response(serializer.data)
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        logger.info(
            f"Пользователь {request.user.username} запросил задачу id={instance.id}"
        )
...
class FriendshipViewSet(viewsets.ViewSet):
    ...
    def send(self, request, pk=None):
        ...
        if (
            Friendship.objects.filter(user=request.user, friend=recipient).exists()
            or Friendship.objects.filter(user=recipient, friend=request.user).exists()
        ):
            return Response(
        ...
        try:
            with transaction.atomic():
                friendship = Friendship.objects.create(
                    user=request.user, friend=recipient, status="PENDING"
                )
            ...
            logger.error(
                f"Error sending friend request from {request.user.id} to {user_id}: {e}",
                exc_info=True,
            )
            return Response(
    ...
    def accept(self, request, pk=None):
        ...
        friendship = get_object_or_404(
            Friendship, user__id=sender_id, friend=request.user, status="PENDING"
        )
        ...
        try:
            with transaction.atomic():
                ...
                Friendship.objects.create(
                    user=request.user, friend=sender_user, status="FRIEND"
                )
                Notification.objects.create(
            ...
            logger.error(
                f"Error accepting friend request id {friendship.id} by user {request.user.id}: {e}",
                exc_info=True,
            )
            return Response(
    ...
    def reject(self, request, pk=None):
        ...
        try:
            ...
            logger.error(
                f"Error rejecting friend request id {friendship_id_for_log} by user {request.user.id}: {e}",
                exc_info=True,
            )
            return Response(
...
class MessageViewSet(viewsets.ModelViewSet):
    ...
    def thread(self, request):
        ...
        return (
            page
            and self.get_paginated_response(serializer.data)
...
def summary(self, request):
    ...
    daily = (
        self.get_queryset()
        .filter(consumed_at__date=today)
        .aggregate(
            total_calories=Sum("calories"),
            total_proteins=Sum("proteins"),
            total_fats=Sum("fats"),
...
class ConsumedCaloriesViewSet(viewsets.ModelViewSet):
    ...
    def summary(self, request):
        ...
        meals = (
            self.get_queryset().filter(consumed_at__date=today).order_by("-consumed_at")
        )
        meals_data = ConsumedCaloriesSerializer(meals, many=True).data
