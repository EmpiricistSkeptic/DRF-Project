...
class AssistantAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        scenario = None
        ai_response_raw = (
            "[System] A response wasn't generated."  # Дефолт, если ИИ не вызван
        )
        ...
        try:
            ...
            try:
                prompt = template.format(user_message=user_message, **user_context)
                prompt_to_save = prompt
            except KeyError as e:
                logger.error(
                    f"Prompt formating error for the script '{scenario}': key {e}."
                )
                final_response_to_user = (
                    "[System] Internal error: failed to prepare data."
                )
                error_occurred_flag = True
                error_message_text = (
                    f"Prompt formating error for the script: key {e} is missing."
                )
                self._save_chat_history(
                    user,
                    user_message,
            ...
            if (
                ai_response_raw.startswith("[System] Error")
                or ai_response_raw
                == "[System] Cannot process the request at the moment."
                or ai_response_raw
                == "[System] Internal error while processing response."
            ):
                logger.warning(
                    f"The call AI returned an error for {user.id}, parsing the quest is being skipped."
                )
                error_occurred_flag = True
                ...
                try:
                    ...
                    if (
                        ...
                        quest_text_block = ai_response_raw[
                            quest_block_start:quest_block_end
                        ]

                        new_quest_object = self._parse_and_create_quest(
                            quest_text_block, user
                        )

                        if new_quest_object:
                            created_quest = new_quest_object
                            response_before_quest = ai_response_raw[
                                :start_index
                            ].strip()
                            response_after_quest = ai_response_raw[
                                end_index + len(QUEST_END_TAG) :
                            ].strip()
                            confirmation_message = f"[System] The new quest '{new_quest_object.title}' has been added to your journal."
                            parts = [
                                part
                                for part in [
                                    response_before_quest,
                                    confirmation_message,
                                    response_after_quest,
                                ]
                                if part
                            ]
                            final_response_to_user = "\n".join(parts)
                            ...
                            logger.warning(
                                f"Quest block found but not processed for {user.id}. The AI's response remains unchanged."
                            )

                except Exception as e:
                    logger.error(
                        f"Error while parsing the quest block for {user.id}: {e}",
                        exc_info=True,
                    )

                    error_occurred_flag = True
                    error_message_text = (
                        error_message_text + "\n" if error_message_text else ""
                    ) + f"Quest parsing error: {e}"
            ...
            logger.error(
                f"Unexpected global error in AssistantAPIView.post for {user.id}: {e}",
                exc_info=True,
            )
